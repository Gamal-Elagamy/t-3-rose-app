...
export default function OccasionsSection() {
  ...
  return (
    <>
      {/* Occasions Section */}
      <div className="occasions grid grid-cols-1 gap-6 mt-6.25 md:grid-cols-2 lg:grid-cols-3">
        {/* Occasions Item */}
        {occasionsInfo.map((item) => (
          <div key={item.id} className="item relative h-67.75 w-full overflow-hidden rounded-2xl">
            {/* Occasions Overlay */}
            <div
              className="overlay absolute top-0 bottom-0 left-0 right-0 bg-black/20"
              aria-hidden="true"
            />
            ...
            <div className="info absolute bottom-0 p-6">
              {/* Occasions Title */}
              <h3 className="w-fit rounded-full bg-maroon-50 px-2 py-0.5 text-xs font-medium leading-4 text-maroon-600">
