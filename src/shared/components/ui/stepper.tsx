...
function StepperItem({
  ...
  // Calculate the state of the next step (for the separator that comes after this indicator)
  const nextState: StepState | undefined = isLast
    ? undefined
    : step + 1 < activeStep
      ? 'completed'
      : step + 1 === activeStep
  ...
  return (
    <StepItemContext.Provider
      value={{ step, state, isDisabled: disabled, isLoading, isLast, nextState }}
    >
      <div
  ...
  const btnRef = useCallback(
    (node: HTMLButtonElement | null) => {
      if (node) {
        ...
        setMyIdx(triggerNodes.length);
      } else {
        registerTrigger(null);
        setMyIdx(-1);
      }
    },
    [registerTrigger, triggerNodes.length]
  );
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-full text-sm font-medium',
        'bg-gray-200 text-gray-400',
        'data-[state=completed]:bg-ds-bg-primary data-[state=completed]:text-white',
        'data-[state=active]:bg-ds-bg-primary data-[state=active]:text-white',
        className
      )}
    >
...
function StepperSeparator({
  className,
  position = 'next',
}: React.ComponentProps<'div'> & { position?: 'prev' | 'next' }) {
  const { state, isLast } = useStepItem();

  // Determine if separator should be primary
  let isPrimary = false;

  if (position === 'prev') {
    // Separator before the indicator
    isPrimary = state === 'active' || state === 'completed';
  } else {
    // Separator after the indicator
    if (state === 'completed') {
      isPrimary = true;
    } else if (state === 'active') {
      // For active step, next separator is muted unless it's the last step
      isPrimary = isLast;
    }
  }
  ...
  return (
    <div
      ...
      className={cn(
        'rounded-full',
        'bg-ds-bg-muted',
        'data-[primary=true]:bg-ds-bg-primary',
        'group-data-[orientation=horizontal]/stepper-nav:h-1 group-data-[orientation=horizontal]/stepper-nav:flex-1',
        'group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-1',
        className
      )}
    />
  );
